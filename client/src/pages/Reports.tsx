...
export default function ReportsPage() {
  ...
  const transactions = allTransactions.filter(
    t => t.budgetId === currentBudgetId && t.payee !== 'Opening Balance' && !t.autoGeneratedById
  );
  ...
  // Calculate income breakdown across the full selected date range
  const incomeBreakdown = React.useMemo(() => {
    const rangeStart = startOfMonth(monthsData[0].date);
    const rangeEnd = endOfMonth(monthsData[monthsData.length - 1].date);
    ...
    transactions
      .filter(t => {
        const txDate = parseISO(t.date);
        return t.amount > 0 && txDate >= rangeStart && txDate <= rangeEnd;
      })
      .forEach(t => {
        if (t.categoryId) {
          const category = categories.find(c => c.id === t.categoryId);
          ...
          // Group uncategorized income by payee so each source is distinct
          const key = `payee:${t.payee}`;
          if (!incomeTotals[key]) {
            incomeTotals[key] = {
              name: t.payee,
              amount: 0,
          ...
          incomeTotals[key].amount += t.amount;
        }
      });
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Last {dateRange} months breakdown</CardDescription>
          </CardHeader>
          <CardContent className="h-[350px]">
