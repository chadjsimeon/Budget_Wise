    ...
    <div className={cn("w-64 bg-sidebar text-sidebar-foreground flex flex-col h-screen border-r border-sidebar-border", className)}>
      ...
      <div className="px-3 mb-4 relative">
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 rounded-md hover:bg-sidebar-accent/30 transition-colors text-sidebar-foreground/70 hover:text-white"
        >
          <div className="flex items-center gap-2">
            <HelpCircle className="w-4 h-4" />
            <span className="text-sm">Help Mode</span>
          </div>
          {/* Simple toggle pill */}
          <div className={cn(
            "w-8 h-4 rounded-full transition-colors flex items-center px-0.5",
            helpModeEnabled ? "bg-blue-500" : "bg-sidebar-foreground/20"
          )}>
            <div className={cn(
              "w-3 h-3 rounded-full bg-white shadow transition-transform",
              helpModeEnabled ? "translate-x-4" : "translate-x-0"
            )} />
          </div>
        </button>

        {/* First-login welcome pointer */}
        {!hasSeenWelcome && (
          <div className="mt-1 mx-1 bg-blue-600 text-white rounded-lg px-3 py-2.5 text-xs shadow-lg">
            <div className="flex items-start justify-between gap-2">
              <p className="leading-snug">
                <span className="font-semibold">New to budgeting?</span><br />
                Turn on Help Mode to get step-by-step guidance as you go.
              </p>
              <button
