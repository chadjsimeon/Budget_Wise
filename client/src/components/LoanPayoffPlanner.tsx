...
export function LoanPayoffPlanner({ account, transactions, formatCurrency }: LoanPayoffPlannerProps) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {(() => {
        // Group auto-generated transactions with their parent payments for display
        const principalTxs = transactions.filter(t => t.payee === 'Loan Payment' && t.amount > 0);
        const legacyTxs = transactions.filter(
          t => !t.autoGeneratedById && t.payee !== 'Loan Payment' && t.payee !== 'Interest Charge'
        );

        const paymentGroups = principalTxs.map(pt => ({
          principalTx: pt,
          interestTx: transactions.find(
            t => t.autoGeneratedById === pt.autoGeneratedById && t.payee === 'Interest Charge'
          ),
        }));

        const paymentCount = paymentGroups.length + legacyTxs.length;
                        ...
                        {paymentGroups.slice(0, 12).map(({ principalTx, interestTx }) => {
                          ...
                          return (
                            <div key={principalTx.id} className="grid grid-cols-4 gap-2 py-2 border-b last:border-0 text-sm">
                              ...
                              <span className="text-right font-medium text-green-700">{formatCurrency(principalAmt)}</span>
                              <span className="text-right text-slate-500">{formatCurrency(interestAmt)}</span>
                              <span className="text-right font-semibold text-slate-700">{formatCurrency(total)}</span>
                            </div>
                          );
                        })}

                        {/* Legacy transactions (pre-fix, no breakdown) */}
                        {legacyTxs.slice(0, 12 - Math.min(paymentGroups.length, 12)).map(tx => (
                          <div key={tx.id} className="grid grid-cols-4 gap-2 py-2 border-b last:border-0 text-sm">
                            <span className="text-slate-600">{tx.date}</span>
                            <span className="col-span-2 text-right text-slate-400 text-xs italic">no breakdown</span>
                            <span className="text-right font-semibold text-slate-700">{formatCurrency(Math.abs(tx.amount))}</span>
                          </div>
                        ))}

                        {paymentCount > 12 && (
                          <p className="text-sm text-slate-500 text-center pt-3">
                            And {paymentCount - 12} more payments…
                          </p>
                        )}
                      </div>
                    )}
